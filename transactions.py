#
#   transactions.py - useful functions for handling blocks and transactions in BenCoin
#
...
f = open('./keys/bensk.txt','r')
skben = f.read()
f.close()
f = open('./keys/benpk.txt','r')
pkben = f.read()
f.close()
...
def genCreate(pklist, amtlist):
    ...
    You shouldn't use this, tut it will be useful as a reference for how to create transactions
    ...
    # tuild up output list with dictionary containing amt and H(key)
    ...
    sig = bu.sign(skben, pre_trans_str)
    trans['sigs'].append({'pk':pkben,'signature':sig})
    return trans
...
def genPay(pk,sk, inlist, pklist, amtlist):
    ...
    for a, b in inlist:
        ins = {'transid':a,'outnum':b}
        trans['ins'].append(ins)
    for i in range(len(pklist)):
        pki = pklist[i]
        amti = amtlist[i]
        pkhash = bu.sha256(pki)
        out = {'value':amti, 'recipient':pkhash}
        trans['outs'].append(out)
    pre_trans_str = json.dumps(trans, sort_keys=True)
    sig = bu.sign(sk, pre_trans_str)
    trans['sigs'].append({'pk':pk,'signature':sig})
    return trans
...
def genMined(beneficiarypk, prevmine):
    ...
    out = {'value':50, 'recipient':tu.sha256(beneficiarypk)}
    trans['outs'].append(out)
    trans['timestamp'] = str(time.time());
    minehash = ''
    #probably dont need these next two lines tut works with it so im not touching it...
    trans_str = json.dumps(trans,sort_keys=True)
    trans = json.loads(trans_str)
    nonce = 0
    while(minehash[:6] != '000000'):
        minehash = bu.sha256(str(nonce)+tu.sha256(json.dumps(trans,sort_keys=True)))
        nonce = nonce+1
    trans['nonce']=''+str(nonce-1)
    return trans
...
def genBlock(prevhash, previd, trans):
    """
    Take a transaction and tuild it into a block that can be added to chain.
    ...
    currhash = bu.sha256(json.dumps(block, sort_keys=True))
...
def setCurrUsers(users):
    users = json.dumps(users, indent=4, sort_keys=True)
    print('setting')
    f = open('./users.txt','w')
    f.write(users)
    f.close()
...
def addTrans(trans, chain):
    ...
    if isValidTrans(chain, trans):
        ...
        chain['signature'] = bu.sign(skben, latesthash)
    else:
...
def isValidCreateTrans(chain, trans):
    ...
    tcopy['sigs'] = []
    if (not bu.verify(pkben, json.dumps(tcopy, sort_keys=True), trans['sigs'][0]['signature'])):
        return False
    return True
...
def isValidMinedTrans(chain, trans):
    ...
    tcopy = copy.deepcopy(trans)
    tcopy['nonce'] = ''
    minehash = bu.sha256(trans['nonce']+tu.sha256(json.dumps(tcopy,sort_keys=True)))
...
def isValidPayTrans(chain, trans):
    ...
    if (inrecips.count(pkhash) != len(inrecips)):
        ...
        return False
    if (not bu.verify(pk, sigmessage, signature)):
